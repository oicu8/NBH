...
This will build ntrnbh-qt as well, if the dependencies are met.
 ...
 **For Ubuntu only:** db4.8 packages are available [here](https://launchpad.net/~ntrnbh/+archive/ntrnbh).
 You can add the repository using the following command:

    sudo apt-get install software-properties-common
    sudo add-apt-repository ppa:ntrnbh/ntrnbh
    sudo apt-get update
...
If you want to build ntrnbh-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a ntrnbh-qt executable will be
built by default.
...
Notes
-----
The release is built with GCC and then "strip ntrnbhd" to strip the debug
