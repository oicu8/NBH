...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::NTRNBH);
    ...
    if (settings.contains("nAnonymizePivxAmount") || reset)
        SoftSetArg("-anonymizentrnbhamount", settings.value("nAnonymizePivxAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in ntrnbh.cpp during shotdown to also remove the window positions
