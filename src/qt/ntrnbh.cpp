...
#include "config/ntrnbh-config.h"
#endif
...
#include "qt/ntrnbh/splash.h"
#include "qt/ntrnbh/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("ntrnbh-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in ntrnbh.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in ntrnbh.qrc)
...
/** Class encapsulating NeutronBH Core startup and shutdown.
...
public:
    ...
    /// Get window identifier of QMainWindow (NTRNGUI)
...
{
    window = new NTRNGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &NTRNGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &NTRNGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(NTRNGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(NTRNGUI::DEFAULT_WALLET);
        ...
        // NeutronBH: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &NTRNGUI::handlePaymentRequest);
        connect(window, &NTRNGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. NeutronBH can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(ntrnbh_locale);
    Q_INIT_RESOURCE(ntrnbh);
    ...
    /// 6. Determine availability of data directory and parse ntrnbh.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("NeutronBH"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("NeutronBH"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // ntrnbh: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("NeutronBH didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
