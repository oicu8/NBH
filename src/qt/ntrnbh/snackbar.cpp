...
#include "qt/ntrnbh/forms/ui_snackbar.h"
#include "qt/ntrnbh/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(NTRNGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &NTRNGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
