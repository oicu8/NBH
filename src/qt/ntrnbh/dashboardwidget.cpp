...
#include "qt/ntrnbh/dashboardwidget.h"
#include "qt/ntrnbh/forms/ui_dashboardwidget.h"
#include "qt/ntrnbh/sendconfirmdialog.h"
#include "qt/ntrnbh/txrow.h"
...
DashboardWidget::DashboardWidget(NTRNGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of NTRN earned via Staking & Masternodes"));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquarePiv, "square-chart-ntrnbh");
    setCssProperty(ui->labelPiv, "text-chart-ntrnbh");
    ...
    setCssProperty(ui->labelAmountPiv, "text-stake-ntrnbh-disable");
	ui->labelAmountMNRewards->setText("0 NTRN");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &NTRNGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
 ...
 //       connect(ui->pushImgEmpty, &QPushButton::clicked, window, &NTRNGUI::openFAQ);
 ///       connect(ui->btnHowTo, &QPushButton::clicked, window);
    ...
    // update the display unit, to not use the default ("NTRN")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair NTRN, zNTRN
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            ntrnbh = (pair["ntrnbh"] != 0) ? pair["ntrnbh"] / 100000000 : 0;
            mnrewards = (pair["mn"] != 0) ? pair["mn"] / 100000000 : 0;
            chartData->totalPiv += pair["ntrnbh"];
            chartData->totalMNRewards += pair["mn"];
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesPiv.append(ntrnbh);    
        chartData->valuesMNRewards.append(mnrewards);    

        int max = std::max(ntrnbh, mnrewards);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    if (chartData->totalPiv > 0 || chartData->totalMNRewards > 0) {
        setCssProperty(ui->labelAmountPiv, "text-stake-ntrnbh");
