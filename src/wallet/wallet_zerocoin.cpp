...
#include "zntrnchain.h"
#include "zntrn/deterministicmint.h"
...
bool CWallet::AddDeterministicSeed(const uint256& seed)
{
    ...
    if(IsCrypted()) {
        ...
        strErr = "save zNTRNseed to wallet";
    }
    //the use case for this is no password set seed, mint dzNTRN,
...
{
    //If this is a zapwallettx, need to read zNTRN
                                            ...
                                            const CCoinControl* coinControl)
{
    ...
    while (true) {
        ...
        if (!CreateZPIVOutPut(denomination, outMint, dMint)) {
            strFailReason = strprintf("%s: failed to create new zNTRN output", __func__);
    ...
    //also assume that a zerocoinspend that is minting the change will not have any change that goes to NTRN
        ...
        std::list<std::pair<CTxDestination, CAmount>> addressesTo, CTxDestination* changeAddress)
{
    ...
    if (res.status != CWallet::CommitStatus::OK) {
        ...
        for (CZerocoinMint mint : vMintsSelected) {
            ...
            zntrnTracker->SetPubcoinNotUsed(hashPubcoin);
    ...
    for (CZerocoinMint mint : vMintsSelected) {
        uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
        zntrnTracker->SetPubcoinUsed(hashPubcoin, txidSpend);

        CMintMeta metaCheck = zntrnTracker->GetMetaFromPubcoin(hashPubcoin);
        if (!metaCheck.isUsed) {
    ...
    receipt.SetStatus("Spend Successful", ZPIV_SPEND_OKAY);  // When we reach this point spending zNTRN was successful
        ...
        CTxDestination * changeAddress)
{
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the smallest available denomination (= 1) of zNTRN"), nStatus);
        return false;
    }
    ...
    const int nMaxSpends = Params().GetConsensus().ZC_MaxPublicSpendsPerTx; // Maximum possible spends for one zNTRN public spend transaction
    std::vector<CMintMeta> vMintsToFetch;
    if (vSelectedMints.empty()) {
        //  All of the zNTRN used in the public coin spend are mature by default (everything is public now.. no need to wait for any accumulation)
        setMints = zntrnTracker->ListMints(true, false, true, true); // need to find mints to spend
        if(setMints.empty()) {
        ...
        // Select the zNTRN mints to use in this spend
    ...
    for (CZerocoinMint mint : vSelectedMints) {
        ...
        if (IsSerialInBlockchain(mint.GetSerialNumber(), nHeightSpend)) {
            ...
            if (!zntrnTracker->HasSerialHash(hashSerial))
                return error("%s: tracker does not have serialhash %s", __func__, hashSerial.GetHex());

            CMintMeta meta = zntrnTracker->Get(hashSerial);
            meta.isUsed = true;
            zntrnTracker->UpdateState(meta);
    ...
    wtxNew.BindWallet(this);
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            for (std::pair<CTxDestination,CAmount> pair : addressesTo){
                ...
                //add output to NTRN address to the transaction (the actual primary spend taking place)
                // TODO: check value?
...
{
    if (fMatureOnly) {
        // This code is not removed just for when we back to use zNTRN in the future, for now it's useless,
        ...
        CAmount nBalance = 0;
        std::vector<CMintMeta> vMints = zntrnTracker->GetMints(true);
    ...
    return zntrnTracker->GetBalance(false, false);
}

CAmount CWallet::GetUnconfirmedZerocoinBalance() const
{
    return zntrnTracker->GetUnconfirmedBalance();
}
...
std::map<libzerocoin::CoinDenomination, CAmount> CWallet::GetMyZerocoinDistribution() const
{
    ...
    {
        LOCK(cs_wallet);
        std::set<CMintMeta> setMints = zntrnTracker->ListMints(true, true, true);
...
void CWallet::setZWallet(CzNTRNWallet* zwallet)
{
    this->zwallet = zwallet;
    zntrnTracker = std::unique_ptr<CzNTRNTracker>(new CzNTRNTracker(this));
}

CzNTRNWallet* CWallet::getZWallet()
{
    return zwallet;
}
...
{
    return zntrnTracker->HasSerial(bnSerial);
}

bool CWallet::IsMyMint(const CBigNum& bnValue) const
{
    if (zntrnTracker->HasPubcoin(bnValue))
        return true;
...
void CWallet::ReconsiderZerocoins(std::list<CZerocoinMint>& listMintsRestored, std::list<CDeterministicMint>& listDMintsRestored)
{
    ...
    for (CZerocoinMint mint : listMints) {
        ...
        if (!zntrnTracker->UnArchive(hashPubcoin, false)) {
            LogPrintf("%s : failed to unarchive mint %s\n", __func__, mint.GetValue().GetHex());
        } else {
            zntrnTracker->UpdateZerocoinMint(mint);
        }
    ...
    for (CDeterministicMint dMint : listDMints) {
        ...
        if (!zntrnTracker->UnArchive(dMint.GetPubcoinHash(), true)) {
...
bool CWallet::SetMintUnspent(const CBigNum& bnSerial)
{
    ...
    CMintMeta meta = zntrnTracker->Get(hashSerial);
    zntrnTracker->SetPubcoinNotUsed(meta.hashPubcoin);
    return true;
}
...
bool CWallet::UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CZerocoinMint mint;
    if (zntrnTracker->HasPubcoinHash(hashValue)) {
        CMintMeta meta = zntrnTracker->GetMetaFromPubcoin(hashValue);
        meta.nHeight = nHeight;
        meta.txid = txid;
        return zntrnTracker->UpdateState(meta);
    } else {
