...
#include "zntrn/zntrnwallet.h"


CzNTRNTracker::CzNTRNTracker(CWallet* parent)
{
    this->wallet = parent;
...
CzNTRNTracker::~CzNTRNTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzNTRNTracker::Init()
{
...
bool CzNTRNTracker::Archive(CMintMeta& meta)
{
...
bool CzNTRNTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzNTRNTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzNTRNTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzNTRNTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzNTRNTracker::GetSerialHashes()
{
...
CAmount CzNTRNTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzNTRNTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzNTRNTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzNTRNTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzNTRNTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzNTRNTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzNTRNTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzNTRNTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzNTRNTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzNTRNTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(wallet->strWalletFile);
...
void CzNTRNTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzNTRNWallet* zNTRNWallet)
{
    bool iszNTRNWalletInitialized = (nullptr != zNTRNWallet);
    CMintMeta meta;
    ...
    if (!iszNTRNWalletInitialized)
        zNTRNWallet = new CzNTRNWallet(wallet);
    meta.isSeedCorrect = zNTRNWallet->CheckSeed(dMint);
    if (!iszNTRNWalletInitialized)
        delete zNTRNWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzNTRNTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzNTRNTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzNTRNTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzNTRNTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzNTRNTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzNTRNTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (fUpdateStatus) {
        ...
        LogPrint(BCLog::LEGACYZC, "%s: added %d dzntrn from DB\n", __func__, listDeterministicDB.size());
    }
