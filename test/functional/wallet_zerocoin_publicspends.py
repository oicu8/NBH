...
class ZerocoinSpendTest(PivxTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zntrn_bal, piv_bal):
            zntrn_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zntrn_bal)
            piv_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], piv_bal)
            return zntrn_bal, piv_bal
        ...
        zntrn_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zntrn_balance, 6666)
        ...
        zntrn_balance, balance = check_balances(denom_2, zntrn_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zntrn_balance, balance = check_balances(denom_3, zntrn_balance, balance)
        ...
        _, _ = check_balances(denom_1, zntrn_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
