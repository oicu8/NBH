...
"""Class for ntrnbhd node under test"""
...
class TestNode():
    """A class for representing a ntrnbhd node under test.
    ...
    def __init__(self, i, dirname, extra_args, rpchost, timewait, binary, stderr, mocktime, coverage_dir, use_cli=False):
        ...
        if binary is None:
            self.binary = os.getenv("BITCOIND", "ntrnbhd")
        else:
            self.binary = binary
        self.stderr = stderr
    ...
    def start(self, extra_args=None, stderr=None, *args, **kwargs):
        ...
        self.running = True
        self.log.debug("ntrnbhd started, waiting for RPC to come up")

    def wait_for_rpc_connection(self):
        """Sets up an RPC connection to the ntrnbhd process. Returns False if unable to connect."""
        # Poll at a rate of four times per second
        poll_per_s = 4
        time.sleep(5)
        for _ in range(poll_per_s * self.rpc_timeout):
            assert self.process.poll() is None, "ntrnbhd exited with status %i during initialization" % self.process.returncode
            try:
        ...
        raise AssertionError("Unable to connect to ntrnbhd")
    ...
    def node_encrypt_wallet(self, passphrase):
        ...
        This causes ntrnbhd to shutdown, so this method takes
