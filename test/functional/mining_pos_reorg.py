...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zntrn):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected NTRN and zNTRN supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zNTRNsupply']:
                assert_equal(node['zNTRNsupply'][denom], DecimalAmt(expected_zntrn[denom]))
    ...
    def run_test(self):
        ...
        # Check NTRN and zNTRN supply at the beginning
        # ------------------------------------------
        # zNTRN supply: 2 coins for each denomination
        expected_zntrn_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # NTRN supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_zntrn_supply)
        ...
        # Verify that NTRN and zNTRN supplies were properly updated after the spends and reorgs
        self.log.info("Check NTRN and zNTRN supply...")
        ...
        expected_zntrn_supply[str(spent_coin_0)] -= spent_coin_0
        expected_zntrn_supply[str(spent_coin_1)] -= spent_coin_1
        expected_zntrn_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_zntrn_supply)
